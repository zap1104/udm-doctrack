...
def test_title_match_outranks_a_body_match(users, offices):
    strong = make_document(offices["MED"], "Preventive maintenance of campus equipment")
    weak = make_document(
...
def test_relevance_is_a_percentage_between_0_and_100(users, offices):
    make_document(offices["MED"], "Electrical supplies request")
    response = search_documents(user=users["admin"], query="electrical supplies", min_relevance=0, log=False)
...
def test_threshold_hides_weak_matches_but_reports_the_count(users, offices):
    ...
    response = search_documents(user=users["admin"], query="electrical supplies", min_relevance=75, log=False)
...
def test_results_respect_document_permissions(users, offices):
    make_document(offices["MED"], "Engineering equipment inventory")
    response = search_documents(user=users["hr"], query="engineering equipment", min_relevance=0, log=False)
    # An HR user has no claim on an Engineering office document unless it was shared.
...
def test_matched_in_explains_why_a_result_appeared(users, offices, tag_urgent):
    document = make_document(offices["MED"], "Urgent request for spare parts")
